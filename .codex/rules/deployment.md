...
- `.github/workflows/release-extension.yml`
  ...
  - GitHub Release asset 게시 후 Chrome Web Store publish job을 분리해서 실행한다.
  - 수동 실행 시 `publish_chrome_web_store=false`를 주면 GitHub Release asset 게시까지만 수행한다.
...
- Extension package release와 Chrome Web Store publish job은 GitHub `extension-release` environment를 사용한다.
...
`extension-release` environment 필수 secrets 또는 variables:

- `CWS_EXTENSION_ID`: Chrome Web Store extension item ID
- `CWS_PUBLISHER_ID`: Chrome Web Store publisher ID

`extension-release` environment 인증 secrets는 아래 방식 중 하나를 사용한다.

- direct access token 방식: `CWS_ACCESS_TOKEN`
- OAuth refresh token 방식: `CWS_CLIENT_ID`, `CWS_CLIENT_SECRET`, `CWS_REFRESH_TOKEN`
- service account 방식: `CWS_SERVICE_ACCOUNT_JSON`

`extension-release` environment 또는 repository variables:

- `CWS_BLOCK_ON_WARNINGS`: publish warning 발생 시 실패할지 여부, 기본값 `true`
- `CWS_PUBLISH_TYPE`: Chrome Web Store publish type, 기본값 `DEFAULT_PUBLISH`
- `CWS_DEPLOY_PERCENTAGE`: optional initial rollout percentage, 지정 시 `0`부터 `100` 사이 정수
- `CWS_SKIP_REVIEW`: review skip 시도 여부, 기본값 `false`
- `CWS_UPLOAD_STATUS_ATTEMPTS`: async upload polling 횟수, 기본값 `18`
- `CWS_UPLOAD_STATUS_INTERVAL_MS`: async upload polling interval milliseconds, 기본값 `10000`
...
- version별 GitHub Release notes는 `apps/extension/release-notes/<manifest-version>.md`에 두며, 파일이 있으면 `release-extension.yml`이 사용한다.
- Chrome Web Store listing 이미지는 `apps/extension/store-assets`에 두고 extension ZIP에는 포함하지 않는다.
- `release-extension.yml`은 GitHub Release asset 게시 후 Chrome Web Store에 package를 upload하고 publish 요청을 제출한다.
- Chrome Web Store V2 `publish` 요청은 release notes를 받지 않으므로 Store listing/release note 성격의 문구는 필요 시 Dashboard에서 별도로 관리한다.
- publish 전에 Chrome Web Store의 published/submitted version을 조회하고, `manifest.json` version이 더 크지 않으면 실패해야 한다.
- package upload 후 publish는 Store review 제출이며, workflow 성공을 Store 즉시 반영으로 해석하지 않는다.

## Chrome Web Store Automation Guardrails

Chrome Web Store 자동 게시를 유지할 때는 아래를 따른다.
